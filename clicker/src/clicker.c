...
void Clicker_ProcessAuto(ClickerContext* clicker, float deltaTime) {
    ...
    if (clicker->autoTimer >= 1.0f) { // Toutes les 1 seconde
...
void Clicker_UpdateMine(ClickerContext* clicker, int viewStartX, int viewWidth, int screenHeight, Font font) {
    ...
    // --- COLONNE 1 : FER ---
    if (DrawAndCheckButtonCentered(font, "  [=====]  \n /// FER \\\\\\", col1Center, artY, 30, LIGHTGRAY)) 
        clicker->inventory.fer++;
        
    // Coût du mineur de fer : 10 Fer
    int costFer = 10 + (clicker->inventory.auto_fer * 5); 
    char buyFerText[50];
    sprintf(buyFerText, "Acheter Mineur\n(-%d Fer)", costFer);
...
void Clicker_RenderMine(ClickerContext* clicker, int viewStartX, int viewWidth, int screenHeight, Font font) {
    ...
    sprintf(statText, "Mineurs: %d\n(+%d/sec)", clicker->inventory.auto_fer, clicker->inventory.auto_fer);
    DrawAndCheckButtonCentered(font, statText, viewStartX + (colWidth / 2), statsY, 20, DARKGRAY);
    
    sprintf(statText, "Chercheurs: %d\n(+%d/sec)", clicker->inventory.auto_or, clicker->inventory.auto_or);
    DrawAndCheckButtonCentered(font, statText, viewStartX + colWidth + (colWidth / 2), statsY, 20, DARKGRAY);
    
    sprintf(statText, "Extracteurs: %d\n(+%d/sec)", clicker->inventory.auto_cristaux, clicker->inventory.auto_cristaux);
...
void Clicker_UpdateForest(ClickerContext* clicker, int viewStartX, int viewWidth, int screenHeight, Font font) {
    ...
    if (DrawAndCheckButtonCentered(font, "  ,_,  \n ( _ ) \n  ' '  \n HERBES", col1Center, artY, 30, GREEN)) 
        clicker->inventory.herbes++;
        
    // Coût de l'Herboriste : 10 Herbes
    int costHerboriste = 10 + (clicker->inventory.auto_herbes * 5); 
    char buyHerbText[50];
    sprintf(buyHerbText, "Acheter Herboriste\n(-%d Herbes)", costHerboriste);
    ...
    char buyBoisText[60];
    sprintf(buyBoisText, "Acheter Bucheron\n(-%d Bois, -%d Herb)", costBucheronBois, costBucheronHerb);
    ...
    sprintf(buyViandeText, "Acheter Chasseur\n(-%d Bois)", costChasseurBois);
...
void Clicker_RenderForest(ClickerContext* clicker, int viewStartX, int viewWidth, int screenHeight, Font font) {
    ...
    sprintf(statText, "Herboristes: %d\n(+%d/sec)", clicker->inventory.auto_herbes, clicker->inventory.auto_herbes);
    DrawAndCheckButtonCentered(font, statText, viewStartX + (colWidth / 2), statsY, 20, DARKGRAY);
    
    sprintf(statText, "Bucherons: %d\n(+%d/sec)", clicker->inventory.auto_bois, clicker->inventory.auto_bois);
    DrawAndCheckButtonCentered(font, statText, viewStartX + colWidth + (colWidth / 2), statsY, 20, DARKGRAY);
    
    sprintf(statText, "Chasseurs: %d\n(+%d/sec)", clicker->inventory.auto_viande, clicker->inventory.auto_viande);
    DrawAndCheckButtonCentered(font, statText, viewStartX + (colWidth * 2) + (colWidth / 2), statsY, 20, DARKGRAY);
}